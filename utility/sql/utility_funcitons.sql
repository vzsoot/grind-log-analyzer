CREATE FUNCTION diff_by_logger(failed_id integer, success_id integer, logger_name varchar(128)) returns table(row_num bigint, failed_message text, success_message text)
    AS '
    select f.rn as row_num, fm as failed_message, sm as success_message from
...
CREATE FUNCTION diff_by_logger_count(failed_id integer, success_id integer) returns table(failed_logger varchar(128), fc bigint, success_logger varchar(128), sc bigint)
    AS '
